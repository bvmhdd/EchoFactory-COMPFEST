          ...
          const response = await fetch(
            `https://generativelanguage.googleapis.com/v1beta/models/${modelName}:generateContent?key=${apiKey}`,
            {
              ...
              body: JSON.stringify({
                ...
                generationConfig: {
                  temperature: 0.3,
                  maxOutputTokens: 600, // Compact & fast responses
                },
              }),
...
function solveUserQuery(query: string, result: DetectionResult | null): string {
  ...
  // 1. Preventive & Corrective Actions / Steps to Handle (preventif, pencegahan, mitigasi, tindakan, menangani, menanganinya, langkah)
  if (
...
1. **Tindakan Lapangan Segera (Immediate Containment)**:
   ...
   - Lakukan inspeksi visual dan pengukuran getaran kontak lokal sesuai standar **${result.supervisor_view.iso_standard}**.
...
3. Lakukan pelumasan berkala pada rumah bearing sesuai manual manufaktur.`;
  ...
  // 2. Maintenance / Care / Housekeeping (rawat, bersih, pelihara, kebersihan, perawatan, housekeeping)
  if (
    q.includes("rawat") ||
    q.includes("bersih") ||
    q.includes("pelihara") ||
...
2. **Pelumasan Bearing & Alignment (${result.supervisor_view.iso_standard})**: Berikan pelumas/grease standar manufaktur pada rumah bantalan (*bearing housing*) secara teratur dan periksa kelurusan poros (*shaft alignment*).
...
3. Pantau spektrum getaran akustik secara berkelanjutan dengan EchoFactory AI.`;
  ...
  // 3. Repair / Fixing / Solution / Prescription (perbaik, perbaiki, solusi, reparasi, fix, rekomendasi, sop, preskriptif)
  if (
    q.includes("perbaik") ||
    q.includes("solusi") ||
    q.includes("reparasi") ||
...
Unit **${result.machine_id}** saat ini dalam kondisi **NORMAL (PASS)** dengan kesehatan ${result.manager_view.machine_health_percentage}%. Tidak memerlukan perbaikan darurat, cukup lanjutkan perawatan pencegahan (*preventive maintenance*) berkala sesuai ${result.supervisor_view.iso_standard}.`;
  ...
  // 4. Financial / Downtime / Cost / Savings / ROI (biaya, rugi, kerugian, hemat, roi, dolar, uang, financial)
  if (
    q.includes("biaya") ||
    q.includes("rugi") ||
    q.includes("kerugian") ||
...
- **Efisiensi Finansial**: Intervensi terencana memangkas biaya perbaikan darurat hingga 70% dibanding perbaikan setelah kegagalan fatal (*run-to-failure*).`;
  ...
  // 5. How to detect / see abnormal status (bagaimana, cara, melihat, tahu, deteksi, abnormal)
  if (
...
3. **Panel Supervisor Gemini**: AI akan menandai nama kerusakan spesifik (${result.supervisor_view.fault_type}) beserta rekomendasi perbaikannya.`;
  ...
  // 6. Safety / Hazard / Operational Risk (aman, bahaya, resiko, risiko, selamat, safety)
  if (
    q.includes("aman") ||
    q.includes("bahaya") ||
    q.includes("resiko") ||
...
${
  ...
  // 7. RUL / Remaining Useful Life / Longevity (rul, umur, sisa, lama, tahan)
  if (
    q.includes("rul") ||
    q.includes("umur") ||
    q.includes("sisa") ||
...
- **Mitigasi Biaya Downtime**: **$${result.manager_view.estimated_downtime_mitigated_usd.toLocaleString()} USD**.`;
  ...
  // 8. Cause / Why / Reason (kenapa, mengapa, penyebab, alasan, faktor)
  if (
    q.includes("kenapa") ||
    q.includes("mengapa") ||
    q.includes("penyebab") ||
...
Teridentifikasi masalah **${result.supervisor_view.fault_type}** yang disebabkan oleh degradasi komponen mekanis atau friksi berlebih yang melanggar batas standar **${result.supervisor_view.iso_standard}**.`;
  ...
  // 9. Blockchain / Proof Hash / Verification / Warranty (blockchain, hash, polygon, verifikasi, proof, garansi, klaim)
  if (
...
- **Status Audit**: Log inspeksi otomatis memenuhi syarat audit dan perlindungan klaim garansi presisi OEM.`;
    }
  }

  // 10. Direct General Prescriptive Answer
  if (result) {
    const isAbnormal = result.operator_view.condition === "ABNORMAL";
    return `[Gemini 2.0 Flash AI Assistant]
...
Langkah yang direkomendasikan adalah: **${result.supervisor_view.recommended_action}** dengan menerbitkan tiket Work Order **${result.supervisor_view.work_order_draft.wo_id}** guna mencegah downtime senilai **$${result.manager_view.estimated_downtime_mitigated_usd.toLocaleString()} USD**.`
  ...
  return `[Gemini 2.0 Flash AI Assistant]
Mengenai pertanyaan Anda "${query}": Silakan pilih salah satu sampel mesin di panel kiri dan jalankan analisis akustik terlebih dahulu untuk mendapatkan telemetri real-time unit terkait.`;
}
