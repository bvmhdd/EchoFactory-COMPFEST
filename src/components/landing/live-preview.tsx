...
import { Activity, Zap, CheckCircle2, ShieldCheck, ArrowRight, Play, Pause, RefreshCw } from "lucide-react";
...
export function LivePreview() {
  ...
  return (
    <section id="preview" className="py-24 md:py-32 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
        <div>
          ...
          <p className="text-xl sm:text-2xl text-zinc-400 font-normal">
            EchoFactory keeps factory assets in motion.
          </p>
        </div>
        <p className="max-w-md text-sm text-zinc-400 leading-relaxed">
          Transform raw 16kHz industrial audio into continuous STgram-MFN feature embeddings. Real-time deterministic diagnosis without cloud latency.
        </p>
      </div>

      {/* Main High-Contrast Console Mockup Frame */}
      <div className="rounded-2xl border border-[#2A2A2E] bg-[#0A0A0B] shadow-[0_20px_60px_rgba(0,0,0,0.9)] overflow-hidden relative">
        {/* Mockup Window Header */}
        <div className="px-6 py-4 border-b border-[#2A2A2E] bg-[#111113] flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Badge variant="mono" className="text-[11px] font-mono">
        ...
        <div className="p-6 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-6 bg-[#111113]/60 rounded-xl p-5 border border-[#2A2A2E]/60">
            <div>
              <div className="flex items-center justify-between mb-4">
                <span className="text-xs font-mono uppercase tracking-wider text-zinc-400">
                  ACOUSTIC SENSOR INGESTION
                </span>
                <span className="text-xs font-mono text-cyan-400">16,000 Hz PCM</span>
              </div>

              {/* Machine selector tabs */}
              <div className="grid grid-cols-4 gap-1.5 p-1 bg-[#0A0A0B] rounded-lg border border-[#2A2A2E] mb-6">
              ...
              <div className="h-28 bg-[#000000] rounded-lg border border-[#2A2A2E] p-3 flex items-center justify-between gap-1 overflow-hidden relative">
                {Array.from({ length: 36 }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex-1 bg-gradient-to-t from-cyan-600 via-sky-400 to-white rounded-full transition-all duration-150"
                      style={{
                ...
                <div className="absolute top-2 right-2 text-[10px] font-mono text-zinc-400 bg-black/70 px-1.5 py-0.5 rounded">
                  0 dB SNR HARSH NOISE
                </div>
              </div>
            </div>

            {/* Quick Actions */}
            <div className="flex items-center justify-between pt-4 border-t border-[#2A2A2E]/60">
              <button
                onClick={() => setIsPlaying(!isPlaying)}
                className="inline-flex items-center gap-2 text-xs font-mono text-zinc-300 hover:text-white bg-[#1C1C1F] px-3 py-1.5 rounded-lg border border-zinc-700 hover:border-zinc-500 transition-colors"
              >
              ...
              <Link href="/dashboard">
                <span className="text-xs text-cyan-400 hover:text-cyan-300 flex items-center gap-1 font-medium">
                  Open In Console <ArrowRight className="w-3 h-3" />
                </span>
              </Link>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Supervisor Card */}
            <div className="p-4 rounded-xl bg-[#111113]/80 border border-[#2A2A2E] flex flex-col justify-between">
              ...
              <p className="text-xs text-zinc-400 line-clamp-2">
                Ganti part #SKF-6204 & lumasi grease ISO VG 46 dalam 48 jam.
              </p>
            </div>
            ...
            <div className="p-4 rounded-xl bg-[#111113]/80 border border-[#2A2A2E] flex flex-col justify-between">
              ...
              <div className="text-xs text-amber-400 mt-1 flex items-center gap-1">
                <span>⚠️ Medium Operational Risk</span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-[#111113]/80 border border-[#2A2A2E] flex flex-col justify-between">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] font-mono text-cyan-400">Amoy #80002</span>
              </div>
              <div className="font-mono text-xs text-zinc-300 truncate bg-black/60 p-1.5 rounded border border-zinc-800">
                0x8f3c71a9e2d5b6...
              </div>
              <div className="text-[11px] text-emerald-400 flex items-center gap-1 mt-1">
                <CheckCircle2 className="w-3 h-3" /> Tamper-Proof Cryptographic Commitment
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 pt-8 border-t border-[#2A2A2E]/60 text-left">
        <div>
