...
export async function generateGeminiDiagnosis(
  ...
  const promptText = `
Anda adalah Gemini Acoustic AI Diagnostic Core untuk sistem pemeliharaan prediktif EchoFactory berstandar ISO 10816-3 & IATF 16949.
Analisis data telemetri akustik & XAI berikut:
...
- FMEA RPN Score: ${result.supervisor_view.fmea_matrix.rpn_score} (${result.supervisor_view.fmea_matrix.risk_category})
- ESG Excess CO2: +${result.manager_view.esg_forensics.excess_co2_kg_per_day} kg CO2e/hari
...
2. Sebutkan akar masalah, analisis gelombang akustik XAI, kepatuhan ISO 10816-3, mitigasi risiko FMEA, dan kepatuhan K3 LOTO.
...
Terdeteksi lonjakan energi harmonik akustik frekuensi tinggi pada gelombang ${freq} Hz dengan skor anomali ${result.operator_view.anomaly_score.toFixed(4)}. Sinyal mengindikasikan ${result.supervisor_view.fault_type} yang melanggar batas getaran aman ${result.supervisor_view.iso_standard} dengan FMEA RPN ${result.supervisor_view.fmea_matrix.rpn_score} (${result.supervisor_view.fmea_matrix.risk_category}). Sisa Umur Operasional (RUL) diperkirakan tersisa ${result.manager_view.estimated_rul_days} hari dengan beban karbon +${result.manager_view.esg_forensics.excess_co2_kg_per_day} kg CO2e/hari. Direkomendasikan protokol LOTO preskriptif: ${result.supervisor_view.recommended_action}. Mitigasi risiko downtime total senilai $${result.manager_view.estimated_downtime_mitigated_usd.toLocaleString()} USD.`);
  } else {
    return sanitizeCompleteResponse(`[Gemini 2.0 Flash Real-Time Analysis]
Spektrum getaran akustik ${freq} Hz beroperasi pada amplitudo laminar stabil (Skor anomali: ${result.operator_view.anomaly_score.toFixed(4)}). Memenuhi kriteria ${result.supervisor_view.iso_standard} dengan tingkat kesehatan unit ${result.manager_view.machine_health_percentage}% dan efisiensi motor ${result.manager_view.esg_forensics.motor_efficiency_pct}%. FMEA RPN 1 (Optimal). Tidak ada degradasi mekanis terdeteksi. Pertahankan jadwal inspeksi rutin 500 jam kerja.`);
  }
}
